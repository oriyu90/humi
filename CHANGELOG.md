...
### Fixed
- **A code block of 8+ lines (or containing long, wrapping lines) got truncated
  with a trailing "…" in the notes preview — sometimes the whole preview went
  blank.** The preview hosted its SwiftUI content in an `NSHostingView` inside
  an `NSScrollView`, which could hand the content a stale, too-short height and
  force-truncate the text. The preview is now a plain SwiftUI `ScrollView`
  again; scroll-position retention across Edit⇄Preview is reimplemented with
  `ScrollViewReader` block anchors instead of the `NSHostingView` route.
- **Tab strip / Home button hit areas were smaller than their visible bounds** —
  clicks near a tab's edge or on a small `×`/pencil glyph could miss. Hit areas
  now cover the full visible chip/button (never larger than it).

### Added
- Custom app icon (`Assets/AppIcon.icns`, generated by the new
  `Scripts/make-icns.sh` from a 1024×1024 source).
